...
- **Skills:** `.cursor/skills/gitnexus/` — e.g. `gitnexus-guide`, `gitnexus-exploring`, `gitnexus-debugging`, `gitnexus-impact-analysis`, `gitnexus-refactoring`, `gitnexus-cli`.
...
1. **Update `release-notes.txt`**  
   Set the content to: `https://ooolab.atlassian.net/browse/<ticket_ID>`  
   Example: `https://ooolab.atlassian.net/browse/LOE-6146`

2. **Create a new branch**  
   - **Target branch**: Use the target branch the user provides; if none, use `learningos`.  
   - **Branch name**: `<type>/<ticket_ID>` (e.g. `feat/LOE-6144`).  
   - **Type**: Pick one from the reference below (from `.gitlab/merge_request_templates/Default.md`):
   ...
   Example: new feature → branch `feat/LOE-6144` from the target branch.
...
Follow the project's development workflow (see `.cursor/skills/development-workflow/SKILL.md`). Summary:
...
When a task matches one of the following, **delegate to the corresponding subagent**. Use [explicit invocation](https://cursor.com/docs/context/subagents#explicit-invocation): **`/name`** in the prompt (e.g. `/api-integration add the new endpoint`) or natural mention (e.g. “Use the presentation subagent to implement this screen”).

| Task | Invoke with | When to use |
|------|-------------|-------------|
| **API / service layer** | `/api-integration` | New or changed endpoints, API contracts, Retrofit APIs, request/response models, or any work in `packages/service`. (`.cursor/agents/api-integration.md`) |
| **Presentation layer** | `/presentation` | Adding or changing screens, cubits, routes, or views in `lib/screens/`, `lib/widgets/`, or `lib/components/`. (`.cursor/agents/presentation.md`) |
| **Tests** | `/test-writer` | **MANDATORY** after finishing code changes: add or update unit, widget, or integration tests for the changes. (`.cursor/agents/test-writer.md`) |
| **Code review** | `/code-reviewer` | **MANDATORY** after finishing development (including unit tests): run format, analyze, bloc lint, and review all changed code. (`.cursor/agents/code-reviewer.md`) |
